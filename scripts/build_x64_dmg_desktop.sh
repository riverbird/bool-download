#!/usr/bin/env bash
# BoolDownload macOS x64 DMG 打包脚本
# 将 osx-x64 的 publish 散文件封装为带图标、已签名的 .app，并打包为 DMG。
#
# 用法: ./build_x64_dmg_desktop.sh
# 可选环境变量:
#   VERSION  应用版本号 (默认 1.3.0)
#   DMG_OUT  DMG 输出路径 (默认 项目根目录/BoolDownload-osx-x64.dmg)
set -euo pipefail
